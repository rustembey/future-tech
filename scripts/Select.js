            ...
            this.state = this.getProxyState(
                { 
                 ...
                 this.updateTabIndexes()
                    this.bindEvents()
                    this.updateState() //вызов updateState
        }
    }
...
updateDropdownPosition() {
    ...
    const halfViewportWidthX = viewportWidth / 2
    const {width, x} = this.buttonElement.getBoundingClientRect()
    const buttonCenterX = x + width / 2
    const isButtonOnTheLeftSide = buttonCenterX < halfViewportWidthX
...
get isNeedToExpand() {
    const isButtonOnFocus = document.activeElement === this.buttonElement

    return (!this.state.isExpanded && isButtonOnFocus)
}
...
onDocumentClick (event) {
    const { target } = event
    const isOutsideDropdown = target.closest(this.selectors.dropdown) !== this.dropdownElement
...
bindEvents() {
    ...
    document.addEventListener('click', this.onDocumentClick.bind(this)); // Привязываем контекст this
    this.rootElement.addEventListener('keydown', this.onKeyDown);
    this.originalControlElement.addEventListener('chnage', this.onOriginalControlChange);
}
}
